...
class Product(models.Model):
    ...
    author = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, default=1, blank=True)

    @property
    def price(self):
        if self.discount and self.discount_percentage:
            new_price = self.old_price - ((self.discount_percentage/100)*self.old_price)
        else:
